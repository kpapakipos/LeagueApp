...
class NetworkingController: NSObject {
    ...
    static func getSummoner(region: String, summonerName: String, completion: @escaping ([String:Any]) -> Void) {
        ...
        let urlString = URL(string: "https://\(region).api.riotgames.com/lol/summoner/v3/summoners/by-name/\(encodedName)?api_key=\(AppModel.apiKey)")
    ...
    static func getSummonerLeague(region: String, summonerId: UInt64, completion: @escaping ([[String:Any]]) -> Void) {
        let urlString = URL(string: "https://\(region).api.riotgames.com/lol/league/v3/positions/by-summoner/\(summonerId)?api_key=\(AppModel.apiKey)")
        if let url = urlString {
            let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
                if error != nil {
                    print(error)
